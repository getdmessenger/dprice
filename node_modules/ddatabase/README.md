...
Built for sharing large datasets and streams of real time data as part of the [DDatabase Protocol](https://@ddatabase/protocol.org).

``` sh
npm install ddatabase
```

[![Build Status](https://github.com/@ddatabase/protocol/ddatabase/workflows/Build%20Status/badge.svg)](https://github.com/@ddatabase/protocol/ddatabase/actions?query=workflow%3A%22Build+Status%22)

To learn more about how ddatabase works on a technical level read the [Dat paper](https://github.com/datprotocol/whitepaper/blob/master/dat-paper.pdf).
...
## Terminology
 ...
 - **feed**. This is what ddatabases are: a data feed. Feeds are permanent data structures that can be shared on the dat network.
...
const dwebcorePublicKeyBuffer = Buffer.from(myPublicKey.buffer)

const ddatabase = ddatabase(storage, dwebcorePublicKeyBuffer)
```
...
If you are using a P2P swarm like [DSwarm](https://github.com/dwebswarm/dwebswarm) you can know this by checking if the swarm connection is a client socket or server socket. In DSwarm you can check that using [client property on the peer details object](https://github.com/dwebswarm/dwebswarm#swarmonconnection-socket-details--)
